...
namespace TonyDev.Game.Core.Behavior
{
    public class UpgradeManager : NetworkBehaviour
    {
        ...
        private readonly Dictionary<int, Action<UpgradeEntry, GameEntity>> _onPurchaseActions = new(); // (id, purchase function): purchase function to
                                                                                                       // be called when an upgrade is purchased
                                                                                                       
        private readonly Dictionary<int, UpgradeEntry> _createdEntries = new(); // (id, UpgradeEntry): used to access classes corresponding to IDs
        ...
        private void Start()
        {
            Player.LocalInstance.playerMovement.OnPlayerMove += (v) => _moveSinceActive += v.magnitude; // Subscribe event to track player distance moved
            
            // Register the upgrades on Start
            #region Upgrade Registering
            
            RegisterLocalUpgrade(5, ObjectFinder.GetSprite("flamingboot"), "Fire shoes", "Set your feet on fire to test upgrades.", () => true,
                (upgrade, entity) =>
                {
                    entity.CmdAddEffect(new InfernalBootEffect()
                    {
                        DamageMultiplier = 0.1f
                    }, entity);
                });
            
            RegisterLocalUpgrade(0, ObjectFinder.GetSprite("inventoryIcons_8"), "Sonic", "Zoom.", () => true,
                (upgrade, entity) =>
                {
                    entity.CmdAddEffect(new SpeedEffect()
                    {
                        MoveSpeedMultiplier = 5f, Duration = 1000000f
                    }, entity);
                });
        ...
        private int _index = 0; // Index to create unique upgrade IDs. Note: This system requires that upgrades are registered in the same order on all clients.
        ...
        public void CmdNotifyPurchaseGlobal(int id, GameEntity localPlayer) // Called in UpgradeEntry when a global purchase goes through.
        {
            ...
            RpcSetPurchased(id); // Notify to clients that the upgrade has been purchased so it can be removed from the UI
        }
        ...
        {
            RemoveUpgrade(id); // Remove the upgrade on purchase, TODO: May allow certain upgrades to be bought multiple times in the future
        }
        ...
        private void RegisterLocalUpgrade(int scrapCost, Sprite icon, string upgradeName, string description,
            ...
            Action<UpgradeEntry, GameEntity> onPurchaseClient) => RegisterUpgrade(scrapCost, icon, upgradeName,
            description, isPurchasable, onPurchaseClient, true);
        ...
        private void RegisterGlobalUpgrade(int scrapCost, Sprite icon, string upgradeName, string description,
            Func<bool> isPurchasable,
            Action<UpgradeEntry, GameEntity> onPurchaseServer) => RegisterUpgrade(scrapCost, icon, upgradeName,
            description, isPurchasable, onPurchaseServer, false);
        ...
        private void RegisterUpgrade(int scrapCost, Sprite icon, string upgradeName, string description,
            Func<bool> isPurchasable,
            Action<UpgradeEntry, GameEntity> onPurchaseServer, bool local)
        {
            ...
            _createdEntries.Add(id, CreateUIElement(scrapCost, icon, upgradeName, description, isPurchasable, id, local)); // Create the UI element and add it to dictionary
        }
        ...
        private UpgradeEntry CreateUIElement(int scrapCost, Sprite icon, string upgradeName, string description,
            Func<bool> isPurchasable, int callbackId, bool local)
        {
            var go = Instantiate(upgradeEntryPrefab, upgradeEntryParent.transform); // Instantiate UI object under UI transform
            
            var ue = go.GetComponent<UpgradeEntry>();
            ue.Set(scrapCost, icon, upgradeName, description, isPurchasable, callbackId, local); // Pass on values to the UpgradeEntry behavior
