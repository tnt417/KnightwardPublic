...
namespace TonyDev.Game.UI.Tutorial
{
    public class TutorialArrow : MonoBehaviour
    {
        ...
        {
            if (PlayerPrefs.GetInt("demo_tutorial_completed", 0) == 1)
            {
                Destroy(gameObject);
                return;
            }
            ...
            await UniTask.WaitUntil(() =>
                Vector2.Distance(storedPlayerPos, Player.LocalInstance.transform.position) > 0.5f);
            ...
            tutorialText.text =
                "Your only goal is to defend this giant crystal by whatever means necessary. Hold [LEFT-CLICK] to attack.";

            var closestSlime = GameManager.GetEntitiesInRange(Player.LocalInstance.transform.position, 20f)
                .First(ge => ge is Enemy);

            // Pause the wave until you kill the slime and place towers
            WaveManager.Pause();

            _goalPos = () => (Vector2) closestSlime.transform.position + new Vector2(0, 0.9f);
            _goalRot = -90f;
            ...
            tutorialText.text =
                "Luckily you have some towers that will be enough to protect the crystal for you! ...for now.";
            ...
            _goalPos = () =>
            {
                if (InventoryUIController.ActivePanel != "Tower")
                {
                    // Get button's screen position
                    var screenPos =
                        RectTransformUtility.WorldToScreenPoint(null, towerInventoryTransform.transform.position);
                    ...
                    return worldPos + new Vector2(0, 1f);
                }

                if (TowerUIController.Instance.Towers.Count == 2)
                {
                    if (TowerPlacementManager.Instance.Placing)
                    {
                        _goalRot = -90f;

                        return (Vector2) Crystal.Instance.transform.position + new Vector2(3f, 1f);
                    }
                    else
                    {
                ...
                if (TowerUIController.Instance.Towers.Count == 1)
                {
                    if (TowerPlacementManager.Instance.Placing)
                    {
                        _goalRot = -90f;

                        return (Vector2) Crystal.Instance.transform.position + new Vector2(3f, 0f);
                    }
                    else
                    {
            ...
            await UniTask.WaitUntil(() => TowerUIController.Instance.Towers.Count == 1);

            tutorialTextAnimator.Play("TextShow");
            tutorialTextAnimator.Play("TextEmphasize");

            SoundManager.PlaySoundPitchVariant("success", 0.5f, Player.LocalInstance.transform.position, 1.05f,
                1.15f);
            ...
            await UniTask.WaitUntil(() => TowerUIController.Instance.Towers.Count == 0);
            ...
            // Unpause the wave after placing towers successfully
            WaveManager.Pause();

            _goalPos = () =>
            {
                if(Vector2.Distance(Player.LocalInstance.transform.position, dungeonLadderTransform.position) < 10f)
                {
                    _goalRot = -90f;
                    return (Vector2) dungeonLadderTransform.position + new Vector2(0f, 1f);
                }
                
                
                _goalRot = 180f;
                return (Vector2) Player.LocalInstance.transform.position + new Vector2(-8f, 0f);
            };
